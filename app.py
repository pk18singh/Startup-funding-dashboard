...
def load_investor_details(investor):
    ...
    with col1:
        #Biggest Investment
        biggest5_series = df[df['investors'].str.contains(investor)].groupby('startup')['amount'].sum().sort_values(ascending=False).head()
    ...
    with col3:
        ...
        st.subheader('City wise most Investment')
        fig2, ax2 = plt.subplots()
        ax2.pie(city_wise, labels=city_wise.index, autopct="%0.01f%%")
        st.pyplot(fig2)
    with col4:
        stage_wise = df[df['investors'].str.contains(investor)].groupby('round')['amount'].sum()
        st.subheader('Stage wise most Investment')
        fig3, ax3 = plt.subplots()
        ax3.pie(stage_wise, labels=stage_wise.index, autopct="%0.01f%%")
        st.pyplot(fig3)
